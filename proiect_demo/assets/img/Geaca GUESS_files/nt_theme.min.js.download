...
$('button.yc_mini_cart').on("click",function(e){
    e.preventDefault();
    ...
    $.getJSON('/cart.js', function(cart) {
        ...
        if (items >= 5) {
          $('#yc_modal_easy').addClass('show');
        } else {
          $('button.yc_mini_cart').unbind('click').trigger('click');
        }
    });
});

$('button.yos_ckt').on("click",function(e){
    e.preventDefault();
    ...
    $.getJSON('/cart.js', function(cart) {
        ...
        if (items >= 5) {
          ...
          $('button.yos_ckt').unbind('click').trigger('click');
        }
    });
});

$('#yc_modal_easy span.yc_close').on("click",function(e){
	$('#yc_modal_easy').removeClass('show');
});

$('.yc_btn_pop').on("click",function(e){  
  	$('button.yc_mini_cart').unbind('click').trigger('click');
});

$('.yc_btn_pop.cart').on("click",function(e){  
  	$('button.yos_ckt').unbind('click').trigger('click');
});
   ...
   ycShopify = (function () {
      return {
		...
		getToday: function (setday, format) {
          var days = yc_settings.order_dayNames.split(" ");
          ...
          if (format =='today') {
           ...
           var frm = yc_settings.order_date_format,
        ...
        locationPopup: function () {
          var location_version = yc_settings.location_version,
              today = ycShopify.getToday('','today'),
              expires = localStorage.getItem('expires_location_'+location_version);
          if (today > expires && expires !== null){
              localStorage.removeItem('location_'+location_version);
              localStorage.removeItem('expires_location_'+location_version);
          }
          var location = localStorage.getItem('location_'+location_version);
          
          if (location == 'accepted') return;
          var popup = $('#popup_yc');
          popup.addClass('popup-display');
          
          var close = $('#popup_yc .modal-header .close');
          
          close.on("click", function (e) {
              e.preventDefault();
              popup.removeClass('popup-display');
              popup.addClass('popup-hide');
              localStorage.setItem('location_'+location_version, 'accepted');
              localStorage.setItem('expires_location_'+location_version, ycShopify.getToday(yc_settings.expires_location,'today'));
          });
        }
      }
   }());
